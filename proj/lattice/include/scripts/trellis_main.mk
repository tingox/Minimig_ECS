...
$(BOARD)_$(FPGA_SIZE)f_$(PROJECT).bit: $(BOARD)_$(FPGA_SIZE)f_$(PROJECT).config
	LANG=C $(ECPPACK) $(IDCODE_CHIPID) --compress --input $< --bit $@

$(CLK0_FILE_NAME):
	LANG=C $(ECPPLL) $(CLK0_OPTIONS) --file $@
	sed -e "s/module pll(/module $(CLK0_NAME)(/g" $@

$(CLK1_FILE_NAME):
	LANG=C $(ECPPLL) $(CLK1_OPTIONS) --file $@
	sed -e "s/module pll(/module $(CLK1_NAME)(/g" $@

$(CLK2_FILE_NAME):
	LANG=C $(ECPPLL) $(CLK2_OPTIONS) --file $@
	sed -e "s/module pll(/module $(CLK2_NAME)(/g" $@

$(CLK3_FILE_NAME):
	LANG=C $(ECPPLL) $(CLK3_OPTIONS) --file $@
